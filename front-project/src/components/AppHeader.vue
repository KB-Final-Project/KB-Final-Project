      ...
      <nav class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav navbar-nav-scroll me-auto" style="--ar-scroll-height: 520px">
          <li v-for="(item, index) in menuItems" :key="index" class="nav-item" :class="{ dropdown: item.subItems }">
            <a v-if="!item.subItems" class="nav-link" :class="{ active: item.active }" :href="item.href">{{ item.text
              }}</a>
            <template v-else>
              <a class="nav-link dropdown-toggle" :class="{ active: item.active }" href="#" data-bs-toggle="dropdown"
                :data-bs-auto-close="item.autoClose" aria-expanded="false">{{ item.text }}</a>
