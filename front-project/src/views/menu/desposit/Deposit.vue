...
const fetchSavings = async () => {
  ...
  try {
    const params = {
      ...
      bankId: selectedBanks.value.length > 0 ? selectedBanks.value.join(',') : undefined, // 쉼표로 구분된 문자열
      saveTerm: selectedDurations.value.length > 0 ? selectedDurations.value.join(',') : undefined, // 단일 선택으로 변경
      page: currentPage.value,
      limit: LIST_LIMIT,
      interestRateType: selectedInterestTypes.value.length > 0 ? selectedInterestTypes.value.join(',') : undefined // 쉼표로 구분된 문자열
    };

    console.log('Fetching savings with params:', params); // 디버깅 로그

    const response = await axios.get('/api/deposit', {
      params,
      paramsSerializer: params => qs.stringify(params, { arrayFormat: 'comma', encodeValuesOnly: true }) // 배열을 쉼표로 구분된 문자열로 직렬화
    });

    if (response.data && response.data.savings) {
      console.log('Received savings:', response.data.savings); // 디버깅 로그
...
const fetchTopSavings = async () => {
  ...
  try {
    ...
    if (response.data && response.data.length > 0) {
      ...
      // /api/deposit/top에서는 은행 정보를 별도로 처리하지 않음
    } else {
      topSavings.value = [];
...
const fetchAllBanks = async () => {
  try {
    ...
    if (response.data && Array.isArray(response.data)) {
      ...
      console.warn('은행 목록이 비어 있습니다.');
      allBanks.value = [];
    }
  } catch (error) {
    console.error('은행 목록을 가져오는 중 오류 발생:', error);
    allBanks.value = [];
  }
};

// 은행 ID를 통해 은행 이름을 조회하는 함수
...
onMounted(() => {
  fetchAllBanks();      // 은행 목록 가져오기
  fetchSavings();       // 예금 상품 목록 가져오기
  fetchTopSavings();    // 상위 예금 상품 목록 가져오기
});

// 검색어 변경 시 페이지 초기화 및 예금 목록 재조회
watch(searchTerm, () => {
  currentPage.value = 1;
  fetchSavings();
});

// 필터 변경 시 페이지 초기화 및 예금 목록 재조회
